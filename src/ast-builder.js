  ...
  return propertiesUnion.map((property) => {
    if ((property in before) && (property in after)) {
      ...
      if ((before[property] instanceof Object) && (after[property] instanceof Object)) {
        ...
        return { diff: 'updated', property, value: { new: after[property], old: recursivelyMarkUnchanged(before[property]) } };
      } else if (after[property] instanceof Object) {
        // if same property in before is a string and in after is an object
        return { diff: 'updated', property, value: { new: recursivelyMarkUnchanged(after[property]), old: before[property] } };
      }
      ...
      return { diff: 'updated', property, value: { new: after[property], old: before[property] } };
    } else if (property in before) {
      // if property exists only in before
      if (before[property] instanceof Object) {
        // if its value is an object
        return { diff: 'removed', property, value: recursivelyMarkUnchanged(before[property]) };
      }
      // if its value is a string
      return { diff: 'removed', property, value: before[property] };
    }
